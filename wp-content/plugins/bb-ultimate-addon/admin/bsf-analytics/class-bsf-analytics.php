	...
	class BSF_Analytics {
		...
		public function option_notice() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				$notice_string = __( 'Want to help make <strong>%1s</strong> even more awesome? Allow us to collect non-sensitive diagnostic data and usage information. ', 'uabb' );

				if ( is_multisite() ) {
					$notice_string .= __( 'This will be applicable for all sites from the network.', 'uabb' );
				}
				...
				Astra_Notices::add_notice(
					array(
						...
						'message'                    => sprintf(
							...
							sprintf( $notice_string . '<span dir="%2s"><a href="%3s" target="_blank" rel="noreferrer noopener">%4s</a><span>', esc_html( $data['product_name'] ), $language_dir, esc_url( $usage_doc_link ), __( ' Know More.', 'uabb' ) ),
							add_query_arg(
								array(
		...
		public function register_usage_tracking_setting() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				add_settings_field(
					...
					__( 'Usage Tracking', 'uabb' ),       // Field title.
		...
		public function render_settings_field_html( $args ) {
			...
			<label for="<?php echo esc_attr( $args['label_for'] ); ?>">
				...
				echo esc_html( sprintf( __( 'Allow %s products to track non-sensitive usage tracking data.', 'uabb' ), $args['title'] ) );// phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText

				if ( is_multisite() ) {
					esc_html_e( ' This will be applicable for all sites from the network.', 'uabb' );
				}
				?>
			</label>
			<?php
			echo wp_kses_post( sprintf( '<a href="%1s" target="_blank" rel="noreferrer noopener">%2s</a>', esc_url( $args['usage_doc_link'] ), __( 'Learn More.', 'uabb' ) ) );
			?>
			</fieldset>
			<?php
		}
